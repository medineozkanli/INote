...
namespace INote.API.Controllers
{
    ...
    public class NotesController : ApiController
    {
        ...
        public IHttpActionResult DeleteNote(int id)
        {
            ...
            if (note.AuthorId != User.Identity.GetUserId())
            {
                return Unauthorized(); //silmek istediğin yazı sana ait mi değilse yetkin yok demesi lazım.
            }
            db.Notes.Remove(note);
            db.SaveChanges();

            return Ok(note.ToGetNoteDto());//silineni döndürüyo.
        }
    }
}
